...
\begin{figure}[!tb]
   ...
   \caption{Expected pulls of predicted event numbers for DeepCore and PINGU after 3 years. We compare the NSI event count with $\emt=-0.05$
    to the standard interaction count}\label{fig:event_pulls} %TODO: remake these with clearer titles and better labels
\end{figure}
...
\begin{figure}[!tb]
   \begin{center}
    ...
    parameters other than the one displayed in each panel are set to zero.}\label{fig:3D_NO}
 \end{figure}
 ...
 \begin{table}
   ...
   \begin{tabular}{lcccc}
           ...
           \cline { 3 - 5 } Parameter & 90\% CL\hspace{1cm} & $\dm$ & $\theta_{23}$  & $\epsilon$  \\
...
\begin{figure}[!tb]
    \begin{center}
       \includegraphics[width=0.4\linewidth]{figures/joint_3D_NO.pdf}
    \end{center}
    \caption{Minimization of the joint $\chi^2$ from Eq.~\ref{fig:joint_chisq} using the $\Delta \chi^2$ shown in Fig.~\ref{fig:3D_NO}}
    \label{fig:joint_3D}
 \end{figure}
 ...
 \begin{table}
   ...
   \end{tabular}
   \end{center}
   \caption{Joint results from $\chi^2_\{text}$ defined in Eq.~\ref{eq:joint_chisq}. Best fit points for $\dm$ and $\theta_{23}$ are given in units of $\si{10^{-3}\eV\squared}$ and
   degrees, respectively.}
\end{table}
