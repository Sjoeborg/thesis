...
\end{align}
NC denotes the neutral current interaction with 
$f \in \{e,u,d\}$. 

For chirality $X \in \{L,R\}$, the NSI Hamiltonian takes the form 
\begin{align}
   H &= \frac{1}{2E} UM^2U^\dagger + \sqrt{2}G_F n_e \text{diag}(1,0,0) + \sqrt{2}G_F \sum_f n_f \epsilon^{fX}
\end{align}

We have no independent sensitivity for the chirality of $\epsilon^X$, so we sum over these and normalize the fermion number density $n_f$ by
...
so $n_u/n_e \simeq n_d/n_e \simeq 3$.
The effective matter NSI parameters now take the form
\begin{align} \label{eq:epsilon}
    \epsilon_{\alpha\beta} &= \sum_X \sum_f \frac{n_f}{n_e} \epsilon^{fX}_{\alpha\beta} \nonumber \\
...
\end{figure}%TODO:redo with more than 100x100. need high emt and log to show both cascade and track features at the same time.
...
% \begin{figure}[!tb]%TODO: redo this with P+/P-
% \begin{center}
%    \includegraphics[width=0.7\linewidth]{figures/Pmm_asymmetry.pdf}
% \end{center}
% \caption{Expected $\Delta \chi^2$ after three years of data.
%    $\dm$ and $\theta_{23}$ and have been marginalized out, and all other NSI 
%    parameters other than the one shown in each panel are fixed to zero.}\label{fig:Pmm_asymmetry}
% \end{figure}

We can explain the asymmetry by looking looking at the oscillogram of the ratio $P(\emt^-)/P(\emt^+)$ in Fig.~\ref{fig:Pmm_asymmetry}, which is the $P_{\mu\mu}$ survival probability
given $\emt =-0.05$ divided by $P_{\mu\mu}$ for $\emt=0.05$. Muon events are the most abundant in IceCube and DeepCore/PINGU, and it suffices to study
$P_{\mu\mu}$ to explain the asymmetry. We see see that positive $\emt$ generates a slightly higher $P_{\mu\mu}$ for energies around \SI{20}{\GeV} (pink area),
while negative $\emt$ produces higher $P_{\mu\mu}$ for almost all other combinations of ${\Etrue,\ztrue}$. As we see in the flux ratio plot, this muon survival abundance 
is indeed preserved at flux level.

\begin{figure}[!tb]
   \begin{center}
      \includegraphics[width=0.5\linewidth]{figures/emt_events.pdf}
   \end{center}
   \caption{The best-fit event count ratio at each detector for negative and positive $\emt$. PINGU shows promising 
   of distinguishing the sign of $\emt$, but favoring $\emt < 0$ while DeepCore has higher statistics for $\emt>0$ and a poor
   distinction for $\emt<0$}\label{fig:emt_events}%TODO: improve caption?
 \end{figure}
As we see in Fig.~\ref{fig:emt_events}, the binned PINGU event count for $\emt=-0.02$ outnumber the event count with $\emt =0.05$ for
all bins except one, giving a high $\Delta \chi^2$ on both sides and slightly higher for $\emt=-0.02$. DeepCore on the other hand, 
has no bins where the event count for $\emt=-0.02$ surpasses the event count for $\emt=+0.02$, giving weak statistics for the negative side.
...
{\renewcommand{\arraystretch}{1.3}
 \begin{table}
   ...
   the systematic uncertainty scenarios considered as in Sec.~\ref{sec:method}}\label{table:IC_DC_results}
\end{table}
\begin{table}
   \begin{tabular}{lcccc}
      \hline \hline \\
   ...
   the systematic uncertainty scenarios considered as in Sec.~\ref{sec:method}}\label{table:PINGU_joint_results}
\end{table}
