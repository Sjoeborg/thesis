    ...
    for flavor in flavors:
        for En in E_range:
            for zn in z_bins:
                filenames=[]
                try:
                    for file in os.listdir(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}/'):
                        if file.endswith('.npy'):
                            filenames.append(file[0:-4])
                    try:
                        df = pickle.load(open(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}.p','rb'))
                    except (FileNotFoundError,EOFError):
                        df = pd.DataFrame(index=[f'E{En}z{zn}'], dtype='object')

                    for file in filenames:
                        array = np.load(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}/{file}.npy')
                        try:
                            df.insert(loc=0,column=file, value=[array])
                        except ValueError: 
                            if update:  # If entry already exists, overwrite/update it
                                df[file][f'E{En}z{zn}'] = array
                    pickle.dump(df,open(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}.p','wb'))
                    if args.d:
                        for file in filenames:
                            os.remove(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}/{file}.npy')
    ...
    for flavor in flavors:
        for En in E_range:
            for zn in z_range:
                try:
                    try:
                        old_df = pickle.load(open(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}.p','rb'))
                    except FileNotFoundError:
                        old_df = pd.DataFrame(index=[f'E{En}z{zn}'], dtype='object')

                    probs = pickle.load(open(f'./pre_computed/new/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}.p','rb'))
                    old_df.update(probs)
                    pickle.dump(old_df,open(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}.p','wb'))
        ...
        for En in E_range:
            for zn in z_range:
                try:
                    for file in os.listdir(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}/'):
                        if file.endswith('.npy'):
                            os.remove(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}/{file}')
                except FileNotFoundError:
                    pass
def df_to_hdf(En,zn, npoints, param_dict,add_attrs=False):
    for flavor in flavors:
        try:
            df = pickle.load(open(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}.p','rb'))
            ...
            for i in range(len(arrays)): 
                dset = f.create_dataset(f'{args.Ndim}gen/{flavor}/{N}/{hashed_list[i]}', data=arrays[i])
            f.close()
            os.remove(f'./pre_computed/{args.Ndim}gen/{flavor}/{npoints}/E{En}z{zn}.p')
...
if __name__ == '__main__':
    ...
    group = f'{args.Ndim}gen'

    for En in E_range:
        for zn in split_z:
            df_to_hdf(En,zn,group, args.N, None,add_attrs=False)
