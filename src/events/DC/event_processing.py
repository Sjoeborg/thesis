...
def perform_chisq(
    ...
    res = minimize(
        fun=chisq,
        x0=x0,
        args=(events, data, background, z, sigma_a, sigma_b, sigma_g, sigma_syst),
        method="Nelder-Mead",
...
def get_deltachi(H1_list, delta_T, sigma=[0.25, 0.15], f=0.09, x0=[1, 0, 0]):
    ...
    data = np.array(
        [
            get_hist(events2018_DC().query(f"pid==0"), "count_events"),
            get_hist(events2018_DC().query(f"pid==1"), "count_events"),
        ]
    )
    background = np.array(
        [
            get_hist(events2018_DC().query(f"pid==0"), "count_background"),
            get_hist(events2018_DC().query(f"pid==1"), "count_background"),
        ]
    )
    sigma_syst = f * np.array(
        [
            get_hist(events2018_DC().query(f"pid==0"), "abs_uncert"),
            get_hist(events2018_DC().query(f"pid==1"), "abs_uncert"),
        ]
    )
...
def list_of_params_nsi(
    dicta, dm31_range, th23_range, ett_range, emt_range, eem_range, eet_range
):
    ...
    dict_list = [
        ...
        for mt in emt_range
        for tt in ett_range
        for th23 in th23_range
        for dm31 in dm31_range
    ]
    return dict_list
...
def marginalize(
    ...
    if len(th23_range) > 1 and len(dm31_range) > 1:
        marginalized_chisq = marginalize_one(
            marginalize_one(reshaped_chisq, dm31_range, 5), th23_range, 4
        )
    elif len(th23_range) > 1 and len(dm31_range) == 1:
        marginalized_chisq = marginalize_one(reshaped_chisq, th23_range, axis=4)
    else:
    ...
    (
        ...
        best_th23_index,
        best_dm31_index,
    ) = np.unravel_index(best_fit_index, reshaped_chisq.shape)
...
def marginalize_one(y, x_range, axis):
    """
            ...
            for flavor_to in ["e", "m", "t"]:
                try:
                    ...
                    if check:
                        ...
                        raise FileNotFoundError(
                            f'P{flavor_from}{flavor_to} {ndim}gen for pid {pid}, dm={dict["dm_31"]}, th23={dict["theta_23"]}, e_mm={dict["e_mm"]},e_mt={dict["e_mt"]}, not found'
                        )
                return True
...
def get_marginalized_array(
    ...
    chisq, best_fit_index = get_deltachi(
        [H for H in H1], 0, [sigma_a, sigma_b], f, x0=[1, 0]
    )
