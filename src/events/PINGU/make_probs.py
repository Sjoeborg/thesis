...
import numpy as np
import time
from PINGU.main import get_events, get_all_events
...
def precompute_probs(args_tuple, nsi=True):
    ...
    res_track = get_events(Ebin=i,zbin=j,params=params,pid=1,nsi=args.nonsi)
    res_cascade =get_events(Ebin=i,zbin=j,params=params,pid=0,nsi=args.nonsi)
...
if __name__ == '__main__':
    ...
    if args.IO is False:
        print(f'Precomputing NO PINGU probabilities for dm31({dm31_range.min()},{dm31_range.max()},{len(dm31_range)}),',
        ...
        print(f'Precomputing IO PINGU probabilities for dm31({dm31_range.min()},{dm31_range.max()},{len(dm31_range)}),',
