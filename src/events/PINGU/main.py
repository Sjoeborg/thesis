...
def get_events(Ebin, zbin, params, pid, nsi, save=True):
    ...
    for flavor_to, mask in masks.items():
        ...
        try:
            Pe = get_probabilities_PINGU(
                "e", flavor_to, Ebin, zbin, params, anti=False, pid=pid, ndim=3, nsi=nsi
            ).reshape(
                -1,
            )
        except KeyError:
            Pe = generate_probabilities_PINGU(
                "e",
                flavor_to,
                Etrue,
                ztrue,
        ...
        try:
            Pm = get_probabilities_PINGU(
                "m", flavor_to, Ebin, zbin, params, anti=False, pid=pid, ndim=3, nsi=nsi
            ).reshape(
                -1,
            )
        except KeyError:
            Pm = generate_probabilities_PINGU(
                "m",
                flavor_to,
                Etrue,
                ztrue,
    ...
    for flavor_to, mask in anti_masks.items():
        ...
        try:
            Pebar = get_probabilities_PINGU(
                "e", flavor_to, Ebin, zbin, params, anti=True, pid=pid, ndim=3, nsi=nsi
            ).reshape(
                -1,
            )
        except KeyError:
            Pebar = generate_probabilities_PINGU(
                "e",
                flavor_to,
                Etrue,
                ztrue,
        ...
        try:
            Pmbar = get_probabilities_PINGU(
                "m", flavor_to, Ebin, zbin, params, anti=True, pid=pid, ndim=3, nsi=nsi
            ).reshape(
                -1,
            )
        except KeyError:
            Pmbar = generate_probabilities_PINGU(
                "m",
                flavor_to,
                Etrue,
                ztrue,
        ...
        rate_weight[mask] = binned_df[mask]["weight"] * (
            mbar_flux * Pmbar + ebar_flux * Pebar
        )
...
def get_all_events(params, nsi, save=True):
    cascades = np.array(
        [
            get_events(Ebin, zbin, params, 0, nsi, save)
            for Ebin in range(8)
            for zbin in range(8)
        ]
    ).reshape(8, 8)
    tracks = np.array(
        [
            get_events(Ebin, zbin, params, 1, nsi, save)
            for Ebin in range(8)
            for zbin in range(8)
        ]
    ).reshape(8, 8)
