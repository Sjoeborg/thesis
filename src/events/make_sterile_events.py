...
if __name__ == "__main__":
    ...
    print(
        f"Precomputing NO IC events for dm41({dm41_range.min()},{dm41_range.max()},{len(dm41_range)}),",
    ...
    p = Pool()
    data = [
        (i, j, alpha, args.N, p, [False, 0, 0], False, False, False, 4)
        for p in param_list
        for i in range(13)
        for j in range(20)
    ]
    ...
    p.close()
    H1_events = np.array(H1_events_list).reshape(
        len(dm41_range) * len(s24_range), 13, 20
    )
    if args.s34:
        pickle.dump(
            H1_events,
