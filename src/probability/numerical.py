...
def H_2(flavor_from, flavor_to, A=0, params=param_dict):  # giunti 9.73
    ...
    H = np.array(
        [  # kanske samma som U_nu(ndim=2,A)  M @ U_conj +
...
def H_3(A_cc, U, M, params=param_dict):
    A = 1e18 * np.array([[A_cc, 0, 0], [0, 0, 0], [0, 0, 0]])
    U_conj = np.conj(U)
    H = U @ M @ U_conj.T + A
    return H
...
def E_3(A_cc, params):
    H_eps = np.array(
        [
            [params["e_ee"], params["e_em"], params["e_et"]],
...
def E_4(A_cc, A_nc, params):
    H_eps = np.array(
        [
            [params["e_ee"], params["e_em"], params["e_et"], params["e_es"]],
...
def P_num(
    ...
    rtol, atol = (
        tols  # put to rtol 1e-4, atol 1e7 whenplotting for better sims (otherwise P can be slightly above unity.)  At the cost of roughly 2x the time
    )

    if vacuum is True:
        solver = solve_ivp(
            fun=evolve,
            t_span=[0, GeV2tokm1 * L_max],
            method=solver,
            y0=state,
            args=(
                flavor_from,
        ...
        solver = solve_ivp(
            fun=evolve,
            t_span=[0, GeV2tokm1 * baseline(theta_i)],
            method=solver,
            y0=state,
            t_eval=eval_at,
            rtol=rtol,
...
def wrapper(p):
    """
    flavor,E_range,z,anti, params, ndim, nsi
    """
    flavor, E_range, z, anti, params, ndim, nsi = p
    P = P_num_over_E(
        flavor, E=E_range, zenith=z, anti=anti, params=params, ndim=ndim, nsi=nsi
    )
    return P
