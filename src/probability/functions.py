...
ic_params.update(
    {
        "theta_12": np.arcsin(np.sqrt(0.846)) / 2,
        "theta_23": np.arcsin(np.sqrt(0.999)) / 2,
        "theta_13": np.arcsin(np.sqrt(9.3e-2)) / 2,
        "dm_21": 7.53e-5,
        "dm_31": 7.53e-5 + 2.44e-3,
    }
)
ic_params.update(
    {
        "theta_14": 0,
        "theta_24": np.arcsin(np.sqrt(0.10)) / 2,
        "theta_34": 0,
        "dm_41": 4.5,
    }
)
ic_params_nsi = ic_params.copy()
ic_params_nsi.update(
    {
        "e_ee": 0,
        "e_em": 0,
        "e_et": 0,
...
dc_params.update(
    {
        "theta_12": 0.5839958715755919,
        "theta_23": 0.8054035005744429,
        "theta_13": 0.14784723758432042,
        "dm_21": 7.53e-5,
        "dm_31": 7.53e-5 + 2.29e-3,
    }
)
dc_params_nsi = dc_params.copy()
dc_params_nsi.update(
    {
        "e_ee": 0,
        "e_em": 0,
        "e_et": 0,
...
nufit_params_nsi_IO.update(
    {
        "theta_12": 33.45 * np.pi / 180,
        "theta_23": 49.3 * np.pi / 180,
        "theta_13": 8.60 * np.pi / 180,
        "dm_21": 7.42e-5,
        "dm_31": -2.498e-3 + 7.42e-5,
        "e_ee": 0,
        "e_em": 0,
        "e_et": 0,
        "e_mm": 0,
...
def U_4(params=param_dict):
    ...
    """
    return (
        R34_4(params["theta_34"])
        @ R24_4(params["theta_24"])
        @ R14_4(params["theta_14"])
        @ R23_4(params["theta_23"])
        @ R13_4(params["theta_13"])
        @ R12_4(params["theta_12"])
    )
...
def U_3(params=param_dict):
    ...
    """
    return (
        R23_3(params["theta_23"])
        @ R13_3(params["theta_13"])
        @ R12_3(params["theta_12"])
    )


def U_5(params=param_dict):
    return (
        V_matrix(3, 5, n=2, params=params)
        @ V_matrix(2, 5, n=2, params=params)
        @ V_matrix(1, 5, n=2, params=params)
        @ V_matrix(3, 4, n=2, params=params)
        @ V_matrix(2, 4, n=2, params=params)
        @ V_matrix(1, 4, n=2, params=params)
        @ V_matrix(2, 3, n=2, params=params)
        @ V_matrix(1, 3, n=2, params=params)
        @ V_matrix(1, 2, n=2, params=params)
    )
...
def baseline(theta_i):
    ...
    if theta_i <= np.pi / 2:
        r = np.sqrt(
            (r_earth + h) ** 2 - r_earth**2 * np.sin(theta_i) ** 2
        ) - r_earth * np.cos(theta_i)
...
def integrate(array, method="simps", *args):
    ...
    if np.ndim(array) == 1:
        ...
        return simpson(
            simpson(simpson(simpson(array, args[0]), args[1]), args[2]), args[3]
        )
...
if __name__ == "__main__":
    print(
        dc_params["dm_21"]
        * 1e-18
        * np.cos(2 * dc_params["theta_12"])
        / (2 * np.sqrt(2) * GF * 0.5 * N_A * 1 * (1 / GeVtocm1) ** 3)
    )
