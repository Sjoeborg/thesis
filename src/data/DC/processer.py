...
from src.probability.functions import mass_dict, dc_params
...
def get_flux(flavor, E, z, df):
    ...
    """
    z = np.clip(
        z, a_min=-1, a_max=0.95
    )  # I dont have flux for this bin, so just set it to 0.95.
    try:
...
def process_aeff(df_list):
    ...
    DOMeff = NearestNDInterpolator(
        np.array([eff_df.E_avg, eff_df.z_avg]).T, eff_df.DOMeff, rescale=True
    )
    ICEeff = NearestNDInterpolator(
        np.array([eff_df.E_avg, eff_df.z_avg]).T, eff_df.ICEeff, rescale=True
    )
...
def multiply_fluxes(MC_df):
    ...
    rate_weights[e_mask] = MC_df[e_mask]["weight"] * get_flux(
        "e", MC_df[e_mask]["true_energy"], MC_df[e_mask]["true_coszen"], interp_flux
    )
    rate_weights[m_mask] = MC_df[m_mask]["weight"] * get_flux(
        "m", MC_df[m_mask]["true_energy"], MC_df[m_mask]["true_coszen"], interp_flux
    )
    rate_weights[ebar_mask] = MC_df[ebar_mask]["weight"] * get_flux(
        "ebar",
        MC_df[ebar_mask]["true_energy"],
        MC_df[ebar_mask]["true_coszen"],
        interp_flux,
    )
    rate_weights[mbar_mask] = MC_df[mbar_mask]["weight"] * get_flux(
        "mbar",
        MC_df[mbar_mask]["true_energy"],
        MC_df[mbar_mask]["true_coszen"],
        interp_flux,
    )
...
def process_systematics(df, systematics, pid):
    ...
    for key in systematics.keys():
        ...
        correction = get_hist(
            systematics[key].query(f"pid=={pid}"), weight="correction_factor"
        )
