...
def flavor_aeff_df_DC(flavor):
    ...
    new_list = []
    for df in df_list:
        df = df.dropna().reset_index(drop=True).drop(0).convert_dtypes()
...
def systematics2015_DC():
    ...
    best_fit_hole = 0.02
    eval_DOM = lambda p: eval(
        p.replace("^", "**").replace(" x", f"*{best_fit_optical_eff}")
    )
...
def events2018_DC():
    ...
    merged_df = pd.merge(
        df,
        df1,
        on=["pid", "reco_coszen", "reco_energy"],
        suffixes=("_events", "_background"),
    )
    return merged_df
...
def systematics2018_DC():
    ...
    for hyperplane in hyperplanes.values():
        hyperplane["correction_factor"] = (
            hyperplane["offset"]
            + hyperplane["ice_absorption"] * (bestfit["ice_absorption"] - 1.0) * 100.0
            + hyperplane["ice_scattering"] * (bestfit["ice_scattering"] - 1.0) * 100.0
