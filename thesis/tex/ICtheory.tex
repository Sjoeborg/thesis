...
Since the IceCube array is slightly rectangular, the zenith angle affects the cross-sectional area to which the array the neutrinos are exposed to.
While the flux was almost flat in $\ztrue$, the introduction of the zenith dependent $\Aeff$ will make the result slightly more zenith dependent than the flux itself. Energy-wise, the situation here is completely reverse from the flux. Increasing with energy, the effective area of the detector
...
\begin{figure}[ht]
    ...
    \caption{IceCube Monte Carlo, binned in $\Ereco$ and $\zreco$. We compare this with our simulations shown as `Null' in the plots. We 
    see a large deviation for high zenith values, but since these neutrinos have traversed less matter and have a smaller baseline, we deem this to be acceptable.}\label{fig:IC_MC_norm}
    \end{centering}
\end{figure} 
