...
%TODO: Look CKM wikipedia, good explanation. Mention that it is not guaranteed that the mixing matrix that we only have one phase. We could Have three due to Dirac/majorana. Majorana phases are not relevant for oscillations because oscillations are not violating lepton number, but include them in PMNS.
of the form $\sin{(\theta_{ij})}/\cos{(\theta_{ij})} = s_{ij}/c_{ij}$, and one phase of the form $e^{i\delta}$. The mixing angle $\theta_{ij}$ specifies the amount of rotation in the $i-j$ plane, allowing an $i$ eigenstate to mix with a $j$ eigenstate. The physical interpretation of
a mixing angle is then the degree to which the $\nu_i$ mass eigenstate mixes with the $\nu_j$ mass eigenstate. A non-zero mixing angle will then 
cause the flavor eigenstate $\nu_\alpha$ to be a superposition of the mass eigenstates that we allowed to mix. More on this in Section~\ref{sec:prop}.

We construct the PMNS matrix using a standard parametrization as
...
\begin{align}
    P_{\na \to \nb}(t) &= \sum_{k,j} {\bra{\nb}\ket{\na (t)}}^2 \nonumber \\
    &= \abs{U^\dagger_{\a k}U_{\b k}U^\dagger_{\b j}U_{\a j}} e^{-i(E_k-E_j)t}\,.
\end{align}
...
\begin{align}\label{eq:Pab} 
    %TODO: include dcp and don't write so many lines
...
$\nm$ to primarily mix into $\nt$. The mixing matrix from~\ref{PMNS_def} takes the values
...
elements for $P_{\a\b}$ are $U_{\a3}, U_{\a1}$ and $U_{\b3}, U_{\b1}$. %TODO: write whole 4 product here
%instead of U,U and U,U

\begin{figure}[!h]
    %TODO: write pme, pmm, pmt in titles, bigger ticks
    \begin{center}
...
\end{figure}
So far we have discussed neutrinos travelling in vacuum. Now we will concern ourselves with neutrino oscillations in matter.
...
fermions relevant for our matter oscillations. 
...
\begin{figure}
    ...
    \caption{Oscillograms showing neutrino oscillations for all flavors in $E - \cos(\theta_z)$ space. Note the panels are symmetric about the diagonal panels, i.e. $P_{\a\b} = P_{\b\a}$ due to $\delta_{CP} = 0$. The core-mantle boundary from Fig.~\ref{fig:potential} is clearly displayed at $\cos{(\theta_z)} = -0.83$ as a sharp discontinuity for all flavors.}\label{fig:oscillograms}%TODO: redo finer
