...
We consider a right-handed neutrino field, $\nu_R$. It has the usual properties of the conventional left-handed neutrino such as hypercharge and color zero. Moreover, since the electroweak gauge group $\text{SU}(2)_L$ only couple to left-handed particles and right-handed antiparticles, $\nu_R$ transforms as a singlet under the SM symmetry group $\mathrm{SU}(3)_{\mathrm{C}} \times \mathrm{SU}(2)_{L} \times \mathrm{U}(1)_{Y}$. This neutrino field is \emph{sterile} since it doesn't participate any of the SM interactions. 
...
\begin{align}
    U &= R_{23}R_{13,\delta_{\text{CP}}}R_{12} \nonumber \\
      & = 
    \begin{pmatrix}{ccc}1 & 0 & 0 \\ 0 & c_{23} & s_{23} \\ 0 & -s_{23} & c_{23}\end{pmatrix}
\begin{pmatrix}{ccc}c_{13} & 0 & s_{13} e^{-i \delta_{\mathrm{CP}}} \\ 0 & 1 & 0 \\ -s_{13} e^{i \delta_{\mathrm{CP}}} & 0 & c_{13}\end{pmatrix}
\begin{pmatrix}{ccc}c_{12} & s_{12} & 0 \\ -s_{12} & c_{12} & 0 \\ 0 & 0 & 1\end{pmatrix}\,.
\end{align}
...
For $N_e \neq 0$, we have closed form solutions, but they are not considered further here.
...
\begin{figure}
    ...
    \emph{Right panel:} $V_{CC}$ using the PREM density 0.5 electrons per nucleon.}\label{fig:potential}
\end{figure}
...
neutrinos that start at the South Pole are `down-going'. We will refer to the quantity $\cos{(\theta_z)}$ as the \emph{zenith}, while
$\theta_z$ is the \emph{zenith angle}.

Since we are interested in the matter effects, we reserve our IceCube study to up-going neutrinos, i.e. neutrinos with
zenith $-1<= \cos{(\theta_z)}<=0$. We now supplement our probability grid from Fig.~\ref{fig:oscillations} with the zenith dimension,
