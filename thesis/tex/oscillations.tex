...
\subsection{Neutrino mixing and the PMNS matrix}
...
\end{align}
Call $V^{\prime \nu \dagger}_{k \alpha}V^{\prime \ell}_{\alpha \alpha} = U^\dagger_{k \alpha}$. We will refer to the matrix $U$ built by the components $U{k \alpha}$ as the
...
Since we now know how the neutrino mass and flavor eigenstates combine (namely, through the PMNS matrix $U$), and have an expression for the flavor interaction with 
...
\begin{figure}
    \includegraphics[width=0.2\textwidth]{figures/w-boson.pdf} 
    \hspace{2cm}
    \includegraphics[width=0.23\textwidth]{figures/z-boson.pdf}
    \caption{\emph{Left panel:} Charged current weak interaction between an electron neutrino and an electron,
    mediated by either a $W^+$ or $W^-$ boson. \emph{Right panel:} Neutral current weak interaction between a neutrino of any flavor and a Earth-like fermion,
...
suggesting that $\nm$ primarily oscillates into $\nt$, at least in vacuum. We will see that matter effects indeed preserves this ordering, making the $\nm \to \nt$ transition
the most abundant. This means we will be able to constrain any parameters relating to $\nm \to \nt$ oscillations stringently. 
