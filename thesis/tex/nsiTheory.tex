...
We have already seen how the failure of the Standard Model to predict neutrino masses and flavor 
osillations forces us to amend it. We have no reason to believe that the Standard Model 
gauge group is the complete picture. Just as the electroweak theory $SU(2)_L \times U(1)_Y$ is 
spontaneously broken to $U(1)_{EM}$, a higher order theory at a different energy scale with ompletely different
properties might be responsible for the Standard Model that we know. In this sense, the Standard Model might be considered 
an effective low energy theory, which is yiels impressive results in some areas, but failing in others. In fact, 
...
%TODO: moreintro. incompleteness of SM
Following the discussion in~\cite{tommyNSI}, the new parameters $\epsilon$ have the scale
\begin{align}
...
two magnitudes below the standard matter effect. Thus, if we assume the new interactions to arise from a higher-energy theory at or above electroweak scale, 
...
theory will have different interactions than those we presently know. We call these interactions non-standard interactions (NSI).
...
parametrized as model-independent four-fermion effective operators \cite{salvadoNSI,nsiFarzan}.
We write the Lagrangians
\begin{align*}
...
$f \in \{e,u,d\}$. The CC NSI effect affects neutrino production and detection, and will not be considered here. NC NSI affect the matter potential, 
...
% Moreover, it is common to subtract $\emm$ from the diagonal, which
...
\subsection{NSIs in DeepCore and PINGU}
