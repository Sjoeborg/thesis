...
th_12, th_13,th_23,th_34,th_23,th_14, th_24 = sp.symbols('theta_12 theta_13 theta_23 theta_34 theta_23 theta_14 theta_24')
E = sp.symbols('E')
d_ij=sp.symbols('delta_ij')
V_cc, V_nc,= sp.symbols("V_cc,V_nc")
...
def get_H_A(En,params,mtype, anti):
    ...
    if anti:
        ...
        M = sp.diag(0,params['dm_21'],params['dm_31'],params['dm_41'])
    A = sp.diag(2*np.sqrt(2) * GF * 0.5 * N_A * 8.44 * (1/GeVtocm1)**3*En, 0 , 0, 2*np.sqrt(2) * GF * 0.5 * N_A * 8.44 * (1/GeVtocm1)**3/2*En)
    H = 1/(2*E*1e9) * (U @ M @ U.T + A*1e18)
    H = H.subs(E,En)
    return H
