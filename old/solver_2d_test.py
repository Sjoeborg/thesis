...
def evolve(t, state, E, N_e, ndim, N_n=0):
    A_cc = (
        2 * np.sqrt(2) * E * GF * (N_e - N_n / 2)
    )  # giunti 332. same as adding A_nc for s!=0
    ...
    H = (
        ...
        * np.array(
            [
                [-m * np.cos(2 * theta) + A_cc, m * np.sin(2 * theta)],
...
def analytical(E, Lmin, Lmax, N_e, L=None):
    ...
    m_M = np.sqrt((m * np.cos(2 * theta) - A_cc) ** 2 + (m * np.sin(2 * theta)) ** 2)
...
if __name__ == "__main__":
    ...
    t_vac, P_num_M = propagate(
        state, N_e=N_e, E=E, Lmin=0, Lmax=L, t_eval=t_vac, ndim=ndim
    )
