...
def get_aeff_df():
    ...
    for flavor_df in [mc_df[num_mask], mc_df[nuam_mask]]:
        ...
        for i in range(len(z) - 1):
            ...
            aeff, bin_edges_num = np.histogram(
                sub_df.Etrue, weights=sub_df.mcweight, bins=energy_bins_fine
            )
            n_items = len(aeff)
            aeff /= 2.0 * np.pi  # Normalise by steradian
            aeff /= np.diff(bin_edges_num)  # Bin widths
            aeff = (
                aeff / 1e4 / (343.7 * 24 * 3600)
            )  # units to m^2 and livetime in seconds
...
def get_energy_resolutionOLD(Er, GPR_model):
    mu_base_e, std_base_e = GPR_model.predict(
        np.log(Er).reshape(-1, 1), return_std=True
    )
...
def interpolate_true_to_reco(df):
    ...
    inter = interpolate.interp1d(
        df["Etrue"], df["Ereco"], fill_value="extrapolate", kind="linear"
    )
    return inter
...
def interpolate_reco_to_true(df):
    ...
    """
    inter = interpolate.interp1d(
        df["Ereco"], df["Etrue"], fill_value="extrapolate", kind="linear"
    )
    return inter
...
def get_E_prob(deposited, reconstructed):
    ...
    def get_odds_energy(deposited, reconstructed):
        ...
        prob = (
            rtwo
            * (1.0 / np.sqrt(s2))
            * np.exp(-0.5 * ((np.log(reconstructed) - mu) ** 2) / s2)
            / reconstructed
        )
...
def generate_probabilitiesOLD(
    flavor_from, flavor_to, E_range, z_range, params, anti, ndim=4
):
    """
    ...
    try:  # Try to open pre-computed df in order to append the new P's
        df = pd.read_csv(
            file_dir + f"{filename}.csv", index_col=0, header=0, dtype=np.float64
        )
    ...
    for z in z_range:  # Compute probs and insert
        res = wrapper([flavor_from, E_range, z, anti, params, ndim])[
            mass_dict[flavor_to]
        ]
        # print(filename, z, rounded_E_range)
        assert (
            type(np.sum(res)) == np.float64
        ), f"Got {res} as probs for {flavor_from}{flavor_to}, {E_range},{z}"
...
def get_probabilitiesOLD(
    ...
    df = pd.read_csv(
        f"./simulations/test/P{flavor_from}{flavor_to}/{filename}" + ".csv",
        index_col=0,
        header=0,
        dtype=np.float64,
    )
    ...
    try:
        ...
        raise AssertionError(
            "At least one of the energies in E_range are not precomputed"
        )
    assert (
        np.shape(subsetted_df)[1] == len(E_range)
    ), f"{len(E_range)-np.shape(subsetted_df)[1]} of the energies in E_range are not precomputed"
    assert (
        np.shape(subsetted_df)[0] == len(z_range)
    ), f"{len(z_range)-np.shape(subsetted_df)[0]} of the zeniths in z_range are not precomputed"
...
def get_flux_dfOLD():
    ...
    doubled_df_list = double_z_bins(
        df_list[9:21]
    )  # Split the bins in two since IC 2020 has 20 bins instead of 10 in our range.
    single_df = pd.concat(doubled_df_list)  # Make it a single df instead
    df = single_df.query("z_max != 0.1").query(
        "z_min!=-1.1"
    )  # Remove two redundant fluxes
...
def get_energy_resolution_df():
    ...
    nue_high = pd.read_csv(
        file2,
        skiprows=0,
        header=None,
        names=["exponent", "nue_sigma"],
        dtype=np.float64,
        skipinitialspace=True,
        sep=",",
    )
...
def get_Aeff_2015(E_range, z_range, interp_list):
    ...
    for z in z_range:
        ...
        aeff = weight * interp_list[i](E_range) + (1 - weight) * interp_list[i - 1](
            E_range
        )
        result.append(aeff)
    res = np.array(result)
...
def interpolate_energy_resolution(df):
    ...
    inter = interpolate.interp1d(
        df["Etrue"], sigma, fill_value="extrapolate", kind="cubic"
    )
    return inter
...
def compare_angErr():
    ...
    plt.plot(
        np.log10(E_range), z1(np.log10(E_range)), c="r", label="interpolated 2012 data"
    )
...
def interpolate_angErr_2015(df):
    ...
    interp = interpolate.interp1d(
        x, z, bounds_error=False, fill_value="extrapolate", kind="cubic"
    )
    return interp
...
def interpolate_X(CC, NC):
    ...
    for flavor in ["e_X", "m_X", "t_X", "ebar_X", "mbar_X", "tbar_X"]:
        interp_CC = interpolate.interp1d(
            E_CC,
            CC[flavor],
            fill_value=CC[flavor].iloc[-1],
            kind="cubic",
            bounds_error=False,
        )
        interp_NC = interpolate.interp1d(
            E_NC,
            NC[flavor],
            fill_value=NC[flavor].iloc[-1],
            kind="cubic",
            bounds_error=False,
        )
    ...
    CC_df = pd.DataFrame(
        [CC_list], columns=["e_X", "m_X", "t_X", "ebar_X", "mbar_X", "tbar_X"]
    )
    NC_df = pd.DataFrame(
        [NC_list], columns=["e_X", "m_X", "t_X", "ebar_X", "mbar_X", "tbar_X"]
    )
